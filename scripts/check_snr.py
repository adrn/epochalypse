...
THE TEST. For a signal of along-scan rms `A` against per-epoch noise `sigma`
over `N` epochs, fitting away the astrometric solution leaves

    chi2 / (N - k)  ~  1 + (A / sigma)^2

and the recorded metric claims `snr_total = sqrt(N) * alpha / sigma_single`.
So a system with `snr_total = 31` over 100 epochs should show chi2/N of order
1 + 31^2/100 / 2 ~ 5, i.e. residuals two-and-a-bit times their own error bars.
If it instead looks like a flat line drawn from the error bars, something is
wrong -- and the point of this script is to say WHICH thing.
...
`snr_single` divides by `sigma_single`. They agree only if `sigma_calib` and
`sigma_att` agree; `sigma_ratio` below is that check, per system.
"""
...
def audit(truth, arrays, n_companions):
    ...
    # rms of what survives the fit, in units of the reported error
    row["snr_measured"] = np.sqrt(n) * float(np.sqrt(max(row["chi2_astro"] - 1.0, 0.0)))
    ...
    if n_companions:
        ...
        row["chi2_predicted"] = 1.0 + 0.5 * row["snr_total"] ** 2 / n
...
def verdict(row):
    ...
    if row["chi2_predicted"] - 1.0 < 0.25:
...
def report_one(row):
    ...
    if "snr_total" in row:
        ...
        print(f"\n  chi2/dof after the 5-param fit  {row['chi2_astro']:8.3f}")
...
def main(argv=None):
    ...
    print(
        f"\n  sigma_single / sigma_reported: median {frame['sigma_ratio'].median():.4f}, "
        f"range {frame['sigma_ratio'].min():.4f} - {frame['sigma_ratio'].max():.4f}"
    )
    if abs(frame["sigma_ratio"].median() - 1.0) > 0.05:
        print("  ^^ every recorded SNR is off by this factor.")
    ...
    print(
        ...
        f"{'detectable':>11}  {'measured':>9}  {'chi2 k=5':>9}  {'predicted':>9}"
    )
