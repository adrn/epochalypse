...
def star(source_id, *, mass=0.55, radius=0.52):
    ...
    return pd.Series({"gaia_source_id": str(source_id),
                      "mass_interp": mass, "radius_interp": radius,
                      "parallax": 8.1, "sig_AL": 0.21, "sig_cal": 0.11,
                      "sig_att_radec": 0.05,
...
def test_seeding():
    ...
    assert P.system_seed(42, "1_companion", 12345) == P.system_seed(42, "1_companion", "12345")
    # and the streams are separated per population and per master seed
    seeds = {P.system_seed(m, pop, "12345")
             for m in (C.SEED_PLANETS, C.SEED_ASTROMETRY) for pop in C.POPULATIONS}
...
def test_determinism():
    ...
    for population in ("1_companion", "2_companion"):
        first = P.draw_companions(population, star(777), n_transits=44, sigma_single=0.21)
        P.draw_companions(population, star(888), n_transits=44, sigma_single=0.21)
        again = P.draw_companions(population, star(777), n_transits=44, sigma_single=0.21)
        assert first == again, f"{population} draw is not a pure function of the source id"
    # a control population injects nothing
    assert P.draw_companions("0_companion", star(777), n_transits=44, sigma_single=0.21) == []


def test_priors_and_screens():
    """Draw a real sample and check every bound and screen it must satisfy."""
    n = 600
    one = pd.DataFrame([P.draw_companions("1_companion", star(10**15 + i),
                                          n_transits=44, sigma_single=0.21)[0]
                        for i in range(n)])
    ...
    assert counts.max() - counts.min() < 0.10, f"log10(a) not flat per decade:\n{counts}"
...
def test_two_companion_pairs():
    ...
    pairs = [P.draw_companions("2_companion", star(10**15 + i),
                               n_transits=44, sigma_single=0.21) for i in range(300)]
    coplanar = 0
    for inner, outer in pairs:
        ...
        label = P.classify_with_resonance(mu[0], mu[1], inner["sma"], outer["sma"],
                                          inner["ecc"], outer["ecc"])
    ...
    for key in ("sma_1", "sma_2", "snr_total_1", "snr_total_2", "coplanar",
                "near_2_1", "near_3_2", "near_resonance"):
...
def test_mass_radius_interpolation():
    ...
    assert np.allclose(got["mass_interp"][:4],
