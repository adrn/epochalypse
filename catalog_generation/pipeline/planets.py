...
def eggleton_lobe_fraction(q):
    ...
    `q` is that body's mass ratio, M1/M2. Accurate to better than 1% over the
    whole range of q, which is far finer than anything this screen needs.
    """
    q13 = np.cbrt(q)
    q23 = q13 * q13
...
def roche_lobe_min_separation(radius_rsun, mstar_msun, mp_msun, *, factor, rsun_in_au):
    """Smallest separation [AU] at which the star still fits inside its Roche lobe.

    Below this the star overflows and the system is a contact/mass-transferring
    binary rather than a star hosting a companion -- geometrically impossible
    for this catalog, so such draws are rejected. Works out to ~1.2 R_star for a
    negligible companion and ~2.6 R_star for an 80 Mjup companion on the
    lightest host.
    """
    lobe = eggleton_lobe_fraction(mstar_msun / mp_msun)
    return factor * (radius_rsun * rsun_in_au) / lobe
...
def generate_population(config: CatalogConfig, spec: PopulationSpec, *,
    ...
    rstar_arr = catalog["radius_interp"].to_numpy(dtype=float)
    ...
    def draw_one_companion(mstar, rstar, parallax_mas, sigma_single_mas, n_dr4):
        ...
        *total* detectability clears the threshold, accumulating single-transit
        ...
        while drawn < priors.snr_max_draws:
            ...
            if priors.enforce_roche_lobe:
                # reject separations at which the star would overflow its lobe
                accept &= sma >= roche_lobe_min_separation(
                    rstar, mstar, mass_msun,
                    factor=priors.roche_lobe_safety_factor,
    ...
    for i in range(n_stars):
        ...
        if spec.n_companions == 1:
            drawn = draw_one_companion(mstar, rstar, parallax_mas, sigma_single_mas, n_dr4)
            if drawn is None:
                n_no_detectable += 1
                continue
            ...
            rows.append({
                ...
                "period": semimajor_axis_to_period(
                    sma, mstar + mass_pl * priors.mjup_in_msun),
                "alpha_mas": alpha_mas,
            ...
            for _attempt in range(priors.max_stability_retries):
                draw1 = draw_one_companion(mstar, rstar, parallax_mas, sigma_single_mas, n_dr4)
                draw2 = draw_one_companion(mstar, rstar, parallax_mas, sigma_single_mas, n_dr4)
                ...
                mu1 = mass1 * priors.mjup_in_msun / mstar
                mu2 = mass2 * priors.mjup_in_msun / mstar
            ...
            period1 = semimajor_axis_to_period(
                sma1, mstar + mass1 * priors.mjup_in_msun)
            period2 = semimajor_axis_to_period(
                sma2, mstar + mass2 * priors.mjup_in_msun)
            near = {
    ...
    if n_no_detectable:
        print(f"  WARNING: {n_no_detectable} stars skipped (no companion clears "
              ...
              + (" outside the stellar Roche lobe)" if priors.enforce_roche_lobe else ")"))
